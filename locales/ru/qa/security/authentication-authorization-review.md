...
**Аутентификация (AuthN)** устанавливает или подтверждает идентичность субъекта. **Авторизация (AuthZ)** решает, разрешено ли этому субъекту выполнить действие над конкретным ресурсом. Успешный вход не доказывает корректность контроля доступа: важны роль, владелец ресурса, tenant, scope, состояние объекта и серверная проверка каждого запроса.

Статья Евгения Гусинца от 22 января 2026 года прочитана полностью. Она охватывает AuthN/AuthZ, stateful-сессии, stateless-токены, жизненный цикл токена и API keys. Сильная сторона — простая модель для начинающего QA. Ограничение — слишком жёсткие правила для HTTP-статусов, хранения состояния и безопасности ключей.
