...
Рассматривайте обмен с API как запрос и ответ. Вместе проверяйте документированное поведение, возвращённые данные, права и побочные эффекты. Успешный статус сам по себе не доказывает корректность.
...
На изображениях указаны **The Thinking Tester, 2018** и русский перевод **software-testing.ru**. Точный URL исходной статьи и лицензия повторного использования не предоставлены. Атрибуция переписана с изображений и независимо не проверялась. Изображения использованы как входной материал; заметка представляет самостоятельное изложение с авторскими примерами, а не воспроизведение их оформления.

Исправления: заменены определения методов через операции БД; примеры query явно зависят от контракта; разделены 401 и 403; JSON использует корректные кавычки и полные объекты; массивы не ограничены объектами. Менее существенные устаревшие заголовки исключены из вводного справочника.
