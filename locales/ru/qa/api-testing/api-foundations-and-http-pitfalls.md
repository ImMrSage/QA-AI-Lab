...
Сначала разберитесь в контракте, затем проверяйте наблюдаемое поведение. Роли клиента и сервера объясняют, кто запрашивает услугу; HTTP задаёт семантику обмена; контракт приложения определяет допустимые данные и бизнес-результат.

Дополнение к [основам запросов REST API](rest-api-request-basics.md) посвящено архитектуре и критическому разбору двух статей QA4Life. Существующая заметка остаётся справочником методов, заголовков, статусов и синтаксиса JSON.

## Охват источников

Прочитаны тексты обеих статей, включая вопросы собеседования и встроенные примеры. Часть 1 вводит клиент-серверную архитектуру, хранение, DNS, HTTP/HTTPS/FTP и контракты API. Часть 2 описывает запросы и ответы, CRUD, методы, JSON/XML и статусы; её внутренняя нумерация также содержит раздел «Часть 3», прочитанный в составе страницы. Встроенные изображения и остальные части серии в этом разборе не рассматривались. Примеры не отправлялись в действующий сервис.
...
| Упрощение в источнике | Более полезное правило проверки |
| --- | --- |
| POST нельзя кешировать | Ответ POST может кешироваться при определённых условиях; проверяйте реальную политику |
| Тело POST практически не ограничено | Серверы, шлюзы и приложения могут ограничивать размер тела |
| PUT очищает пропущенное поле до null | Семантика замены не задаёт именно такой результат для поля; нужен контракт |
| 403 доказывает успешную аутентификацию | Это отказ; аутентификация не является универсальным предварительным условием |
| 500 доказывает корректность запроса | Это непредвиденный сбой сервера, а не подтверждение валидности входа |
| Любая версия HTTP использует текстовую стартовую строку | Так устроен HTTP/1.x; HTTP/2 использует фреймы и псевдозаголовки |
