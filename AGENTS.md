...
4. Make the material accessible in the website/application through its formatted reader and relevant navigation or collection. Check affected links and rendering. Report any application integration that remains unfinished.
   The reader automatically builds an interactive visual summary from the article's substantive sections. Write concise, descriptive section headings and opening paragraphs so this map remains meaningful. Add an authored Mermaid diagram when sequence, state, architecture, comparison, or data flow is central to understanding; do not add decorative diagrams that merely repeat prose.
