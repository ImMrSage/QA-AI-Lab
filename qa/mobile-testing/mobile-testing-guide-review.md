...
- The list is stale. Android 16 is API 36, and Google Play requires API 36 for new apps and updates from August 31, 2026. Derive iOS coverage from the product's current support matrix and analytics
- They are commonly described as cross-platform native frameworks; hybrid usually refers to web content in a native container. WebView and native-rendering risks differ
- This is a starting heuristic. Include the minimum supported version, user share, target SDK, crash-heavy models and relevant platform changes
- Keychain, cloud backups, server state and some identifiers may remain. Test local, cloud and server state separately
- Limits have changed and depend on OS version and invocation type; use Apple's current requirements for the release
- Apps targeting Android 7+ do not trust user CAs by default. Use a safe debug trust configuration; pinning and individual networking libraries may still block interception
- Android warns about pinning risks and recommends a backup key. Security and architecture owners need a rotation and recovery plan
- These may be targets, but require a product SLO, device class, network, start state, percentile and measurement method
- 4.5:1 applies to normal text; large text may use 3:1, while UI components and graphics have separate WCAG criteria
- The API contract defines status codes, idempotency and response bodies. A repeated DELETE may validly return 204, 404 or another documented response
- This applies only when the contract guarantees idempotency, for example through an idempotency key. Multiple resources may otherwise be correct
- These are security probes. Use them only in an authorized environment and scope without risking data damage
- Use a mixed strategy. Emulators scale quickly; physical devices matter for sensors, OEM behavior, energy, radio conditions and actual performance
